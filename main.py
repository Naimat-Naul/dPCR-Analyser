...
def load_data_from_eds():
    ...
    file_label.config(text=f"Selected File: {current_file.split('/')[-1]}")  # Update label
...
def retrive_data(fam, rox, vic, com):
    ...
    df = data[data['roxP'] == 1].drop(columns=['roxP'])  # Filter rows where roxP == 1
    return df


# Function to process data
def process_data(df):
    df['FVP'] = ((df['famP'] == 1) & (df['vicP'] == 1)).astype(int)  # Both FAM and VIC positive
    df['FVN'] = ((df['famP'] == 0) & (df['vicP'] == 0)).astype(int)  # Both FAM and VIC negative
    df.loc[df['FVP'] == 1, ['famP', 'vicP']] = 0  # Update famP and vicP where FVP is 1
    return df
...
def plot_interactive(df, initial_vic_threshold, initial_fam_threshold):
    ...
    # Initial threshold lines at specified maximums
...
def update_plot(vic_threshold, fam_threshold):
    ...
    # Apply new thresholds and reprocess data
    ...
    try:
        ...
        if os.path.exists(save_path):
            ...
            if file_name in existing_data['File'].values:
                # Update the row where 'File' matches
...
def main_app():
    ...
    # Configure resizing of the canvas
    main_frame.bind(
        "<Configure>",
